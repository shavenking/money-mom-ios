...
class TransactionTableViewCell: UITableViewCell {
  ...
  var transaction: Transaction? {
    didSet {
      ...
      amountLabel.text = transaction.amount == NSDecimalNumber.notANumber
...
extension TransactionTableViewCell: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
  ...
  func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
    guard let cell = (collectionView as? TagCollectionView)?.dequeueReusableCell(forReadOnlyTagAt: indexPath) else {
      fatalError("Can not dequeue readonly cell for TagCollectionView in TransactionTableViewCell")
    }
...
extension TransactionTableViewCell {
  @objc private func userWannaDelete() {
    ...
    UIView.animate(withDuration: 0.2, animations: { () -> Void in
      ...
      if finished, let viewContext = (UIApplication.shared.delegate as? AppDelegate)?.persistentContainer?.viewContext {
...
extension TransactionTableViewCell {
  override func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
    if gestureRecognizer == panGestureRecognizer,
       let velocity = (gestureRecognizer as? UIPanGestureRecognizer)?.velocity(in: contentView) {
