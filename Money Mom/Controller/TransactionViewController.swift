...
class TransactionViewController: UIViewController {
  ...
  lazy var fetchedResultsController: NSFetchedResultsController<Transaction> = {
    ...
    let fetchedResultsController = NSFetchedResultsController<Transaction>(
      fetchRequest: request,
      managedObjectContext: viewContext,
      sectionNameKeyPath: #keyPath(Transaction.createdAtDay),
      cacheName: nil)
  ...
  override func viewDidLoad() {
    ...
    navigationItem.rightBarButtonItem = UIBarButtonItem(barButtonSystemItem: .add,
                                                        target: self,
                                                        action: #selector(showTransactionCreateViewController))
...
extension TransactionViewController: UITableViewDataSource, UITableViewDelegate {
  ...
  func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
    ...
    guard let headerView = (tableView as? TransactionTableView)?.dequeueReusableHeaderView() else {
      fatalError("failed to dequeue HeaderView from TransactionTableView")
    }
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    guard let cell = (tableView as? TransactionTableView)?.dequeueReusableCell(for: indexPath) else {
      fatalError("failed to dequeue cell from TransactionTableView")
    }
