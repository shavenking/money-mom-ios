...
class Version3ToVersion4MigrationPolicy: NSEntityMigrationPolicy {
  ...
  override func createRelationships(forDestination dInstance: NSManagedObject,
    ...
    case "TransactionStats":
      let sTransactionStats = manager.sourceInstances(forEntityMappingName: mapping.name,
                                                      destinationInstances: [dInstance]).first!
      let dateRange = [
        calendar.startOfDay(for: sTransactionStats.value(forKey: "date") as! Date),
        calendar.startOfDay(for: calendar.date(byAdding: .day,
                                               value: 1,
                                               to: sTransactionStats.value(forKey: "date") as! Date)!)
      ]
    ...
    case "Transaction":
      let sTransaction = manager.sourceInstances(forEntityMappingName: mapping.name,
                                                 destinationInstances: [dInstance]).first!
      ...
      let dQuickRecord = manager.destinationInstances(forEntityMappingName: mapping.name,
                                                      sourceInstances: [sQuickRecord as! NSManagedObject]).first
      dInstance.setValue(dQuickRecord, forKey: "quickRecord")
    case "QuickRecord":
      let sInstance = manager.sourceInstances(forEntityMappingName: mapping.name,
                                              destinationInstances: [dInstance]).first!
      ...
      let dInstanceRelationship = manager.destinationInstances(forEntityMappingName: mapping.name,
                                                               sourceInstances: [
                                                                 sInstanceRelationship as! NSManagedObject]).first
  ...
  private func createDestinationTag(named tagName: String, manager: NSMigrationManager) -> NSManagedObject {
    ...
    if tags.isEmpty {
      let tag = NSEntityDescription.insertNewObject(forEntityName: String(describing: Tag.self),
